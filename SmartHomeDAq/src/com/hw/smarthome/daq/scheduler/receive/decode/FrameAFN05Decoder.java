...
public class FrameAFN05Decoder extends FrameDecoder {
	...
	private int decode(List<DataItem> dataItems, DAqPo frame, int pos) {
		byte[] bytes = frame.getReceiveData();
		...
		case 0x01:
			...
			break;
		case 0x02:
			pos = decode02(dataItems, frame, pos);
			break;
		}
		return pos;
	}

	private int decode02(List<DataItem> dataItems, DAqPo frame, int pos) {
		DataItemAFN05FN02 tempItem = new DataItemAFN05FN02();
		tempItem.setFn((byte) 0x01);
		byte[] bytes = frame.getReceiveData();
		tempItem.setControlMode(ByteUtils.getBin(bytes, pos));
		tempItem.setSuccess(true);
	 ...
	 */
	private int decode01(List<DataItem> dataItems, DAqPo frame, int pos) {
		DataItemAFN05FN01 tempItem = new DataItemAFN05FN01();
		tempItem.setFn((byte) 0x01);
		byte[] bytes = frame.getReceiveData();
		boolean success = (1 == ByteUtils.getBin(bytes, pos)) ? true : false;
