...
(function($) {
	...
	function _7(_8) {
		...
		if (_f > _10) {
			...
			_e.css({
				...
				left : 0,
				width : _10 - _c.outerWidth() - _d.outerWidth()
			});
		} else {
			_c.hide();
			_d.hide();
		...
		if (_13.tools) {
			if (typeof _13.tools == "string") {
				...
				for ( var i = 0; i < _13.tools.length; i++) {
					var _16 = $("<a href=\"javascript:void(0);\"></a>")
							.appendTo(_15);
					_16[0].onclick = eval(_13.tools[i].handler || function() {
					});
					_16.linkbutton($.extend({}, _13.tools[i], {
						plain : true
					}));
				}
			}
	...
	function _2a(_2b) {
		...
		$(".tabs-scroller-left", _2d).unbind(".tabs").bind("click.tabs",
				function() {
		...
		$(".tabs-scroller-right", _2d).unbind(".tabs").bind(
				"click.tabs",
				function() {
					var _30 = $(".tabs-wrap", _2d);
					var pos = Math.min(_30.scrollLeft() + _2c.scrollIncrement,
							_1(_2b));
					_30.animate({
						scrollLeft : pos
					}, _2c.scrollDuration);
		...
		for ( var i = 0, len = _31.length; i < len; i++) {
			...
			tab.unbind(".tabs").bind("click.tabs", {
				...
				_44(_2b, _34(_2b, e.data.p));
					...
					function(e) {
						_2c.onContextMenu.call(_2b, e, e.data.p
								.panel("options").title);
					});
	...
	function _35(_36, pp, _37) {
		...
		pp.panel($.extend({}, _37, {
			...
			closed : true,
			doSize : false,
			iconCls : (_37.icon ? _37.icon : undefined),
			onLoad : function() {
				if (_37.onLoad) {
		...
		var _3b = $("<a href=\"javascript:void(0)\" class=\"tabs-inner\"></a>")
				.appendTo(tab);
		var _3c = $("<span class=\"tabs-title\"></span>").html(_38.title)
				.appendTo(_3b);
	...
	function _45(_46, _47) {
		...
		pp.panel($.extend({}, _47.options, {
			iconCls : (_47.options.icon ? _47.options.icon : undefined)
		}));
	...
	$.fn.tabs.parseOptions = function(_77) {
		var t = $(_77);
		return {
			width : (parseInt(_77.style.width) || undefined),
			height : (parseInt(_77.style.height) || undefined),
