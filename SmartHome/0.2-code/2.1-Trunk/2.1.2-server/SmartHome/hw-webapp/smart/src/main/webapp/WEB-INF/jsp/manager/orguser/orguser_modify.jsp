...
<!-- 									cssClass="alnum {required:true,maxlength:20,remote:{type:'post',url:'manager/orguser!checkOnlyLoginName',data:{loginName:function(){return $('#loginName').val();}}},messages:{required:'请输入账号名称',remote:'登录用户名已被使用'}}" -->
							...
							<td class="tb_inputtwo"><s:textfield name="sysUserPO.REAL_NAME"
									id="userName" cssClass="chcharacter {required:true,rangelength:[2,10],messages:{required:'请输入账号名称'}} " maxlength="10"
									/></td>
						</tr>
						<tr>
							<th>年龄</th>
							<td class="tb_inputtwo"><s:textfield name="sysUserPO.AGE"/></td>
							<th>性别</th>
							<td class="tb_inputtwo">
						...
						<tr  id="orgInfoTr">
							...
							<th>员工编号</th>
							<td class="tb_inputtwo">
								<s:textfield id="employeeId" name="sysUserPO.EMPLOYEE_NUM" cssClass="{remote:{type:'post',async:false,url:'manager/orguser!checkOnlyEmployeeId',data:{employeeId:function(){return $('#employeeId').val();},uuid:function(){return $('#uuid').val();},'orgId':function(){return $('#orgId').val();}}},messages:{remote:'员工编号重复'}}"/>
							</td>
						</tr>
						<tr>
							<th>是否管理员</th>
					...
					<ul>
  						<li class="back">
				       	...
				       	<li class="save" style="margin-right:15px;">
						 	<input type="button" class="save" onclick="doSubmitOrgEditData();">
				       	</li>
