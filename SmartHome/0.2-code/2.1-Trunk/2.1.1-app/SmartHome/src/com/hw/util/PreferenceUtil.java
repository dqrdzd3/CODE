...
public class PreferenceUtil {
	...
	//设置和获得当前sensorid显示历史曲线的类型（温度、湿度等）          //shareName 为  s+sensorid 
	public static void saveHisType(Context context,String shareName,
			String tp) {
		SharedPreferences settings = context
				.getSharedPreferences("s"+shareName, 0);
		Editor editor = settings.edit();
		editor.putString("TYPE", tp);
		editor.commit();
	}

	public static String getHisType(Context context,String shareName) {
		SharedPreferences settings = context
				.getSharedPreferences("s"+shareName, 0);
		return settings.getString("TYPE", UIConstant.HOME_NAME_TEMPERATURE);
	}
	//设置和获得历史前多少小时或天数           暂时无用
		public static void saveHisLast(Context context,
				int last) {
			SharedPreferences settings = context
					.getSharedPreferences(PREFS_NAME, 0);
			Editor editor = settings.edit();
			editor.putInt("LASTPAR", last);
			editor.commit();
		}
		...
		public static int getHisLast(Context context) {
			SharedPreferences settings = context
					.getSharedPreferences(PREFS_NAME, 0);
			return settings.getInt("LASTPAR", 7);
		}
		//设置和获得当前sensorid显示历史曲线的类型（温度、湿度等）
//	public static void saveHisType(Context context,
//						int last) {
//					SharedPreferences settings = context
//							.getSharedPreferences(PREFS_NAME, 0);
//					Editor editor = settings.edit();
...
//				}
//
//	public static int getHisType(Context context) {
//					SharedPreferences settings = context
//							.getSharedPreferences(PREFS_NAME, 0);
//					return settings.getInt("LASTPAR", 7);
//				}
	...
	// 保存摄像头是前置还是后置
	public static void saveCameraOriation(Context c,String oriation){
		SharedPreferences settings = c.getSharedPreferences(
				PREFS_NAME, 0);
		Editor editor = settings.edit();
		editor.putString("cam_oriation", oriation);
		editor.commit();
	}
	
	public static String getCameraOriation(Context c){
		SharedPreferences settings = c.getSharedPreferences(
				PREFS_NAME, 0);
		return settings.getString("cam_oriation", "front");
	}
	//结束
	
	// 保存闪光灯是否开启
	public static void saveCameraLight(Context c,boolean is){
		SharedPreferences settings = c.getSharedPreferences(
				PREFS_NAME, 0);
		Editor editor = settings.edit();
		editor.putBoolean("flash_light", is);
		editor.commit();
	}
	
	public static boolean getCameraLight(Context c){
		SharedPreferences settings = c.getSharedPreferences(
				PREFS_NAME, 0);
		return settings.getBoolean("flash_light", true);
	}
		//结束
}
