...
public class Strings {
	...
	public static <T> String joinAnd(final String delimiter,
		...
		final StringBuilder buffer = new StringBuilder(
				Strings.toString(iter.next()));
		int i = 1;
		while (iter.hasNext()) {
			final T obj = iter.next();
			if (notEmpty(obj))
				buffer.append(
						++i == objs.size() ? lastDelimiter
								: delimiter).append(
	...
	public static <T> String joinAnd(final String delimiter,
		...
		return joinAnd(delimiter, lastDelimiter,
				Arrays.asList(objs));
	}
		...
		return o == null ? def
				: o instanceof InputStream ? toString((InputStream) o)
						: o instanceof Reader ? toString((Reader) o)
								: o instanceof Object[] ? Strings
										.join(", ", (Object[]) o)
										: o instanceof Collection ? Strings
												.join(", ",
														(Collection<?>) o)
												: o.toString();
	}
	...
	public static String md5(String s) {
		...
		try {
			...
			final byte[] hash = MessageDigest.getInstance("MD5")
			...
			for (byte aHash : hash) {
				...
				if (hex.length() == 1) {
					hashString.append('0');
					hashString
							.append(hex.charAt(hex.length() - 1));
				} else {
					hashString
							.append(hex.substring(hex.length() - 2));
				}
			}
	...
	public static String[] chunk(String str, int chunkSize) {
		...
		for (int i = 0; i < arrayLen; ++i)
			array[i] = str
					.substring(i * chunkSize, (i * chunkSize)
							+ chunkSize < len ? (i * chunkSize)
							+ chunkSize : len);
